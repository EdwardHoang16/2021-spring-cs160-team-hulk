...
class Map extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (
      (center.lat !== location.lat || center.lng !== location.lng) &&
      (shouldApplyProps || _selectedFarm !== selectedFarm) &&
      !isSearching
    ) {
      return {
   ...
   * @memberof Map
   */
  updateLocation = memoizeOne((marker, _location) => {
          ...
          <div className="map-wrapper">
            {mapsApiLoaded && (
              <FarmSearchBox
                map={mapInstance}
                mapsapi={mapsapi}
                onPlacesChanged={(data, ref) => this.handleSearch(data, ref)}
              />
            )}
            <button className="btn-marker" onClick={() => this.getRedMarkerBack()}>
            ...
            >
              <RedMarker lat={_location.lat + 0.001} lng={_location.lng} />
              {markers}
            </GoogleMapReact>
          </div>
        </Paper>
